...
export const changeStake = createAsyncThunk(
  ...
  async ({ action, value, provider, address, networkID }: IActionValueAsyncThunk, { dispatch }) => {
    ...
    try {
      if (action === "stake") {
        uaData.type = "stake";
        stakeTx = await kageStakingContrat.deposit(ethers.utils.parseUnits(value, "gwei"), address);
      } else {
        uaData.type = "unstake";
        stakeTx = await kageStakingContrat.withDraw(ethers.utils.parseUnits(value, "gwei"), true);
      }
